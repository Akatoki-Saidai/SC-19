...
bool Adafruit_BNO055::getEvent(sensors_event_t *event)
{
  ...
  event->timestamp = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now()-StartSystemTime).count();
...
{
  uint8_t send_data[2] = {static_cast<uint8_t>(reg), value};
  i2c_write_blocking(I2cChannel, BNO055_ADDRESS_A, send_data, 2, false);
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, uint8_t * buffer, uint8_t len)
{
  uint8_t reg_uint = static_cast<uint8_t>(reg);
  i2c_write_blocking(I2cChannel, BNO055_ADDRESS_A, &reg_uint, 1, true);
  int BRead = i2c_read_blocking(I2cChannel, BNO055_ADDRESS_A, buffer, len, false);
